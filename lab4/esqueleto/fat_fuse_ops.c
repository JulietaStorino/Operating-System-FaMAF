...
void fat_fuse_log_activity(char *operation_type, fat_file target_file)
{
    ...
    fat_file_pwrite(log, buf, buf_size, log->dentry->file_size, parent);
}
...
static void fat_fuse_log_init(void) {
    ...
    if (log_node != NULL) {
        DEBUG("/fs.log already exists");
        return;
    }

    DEBUG("/fs.log being created");

    int mknod_exit = fat_fuse_mknod(LOG_FILE, 0, 0);
    if (mknod_exit != 0) {
        DEBUG("/fs.log unable to create");
        return;
    }
    ...
    char operation[5] = "INIT";
    fat_fuse_log_activity(operation, log_file);
...
int fat_fuse_read(const char *path, char *buf, size_t size, off_t offset,
    ...
    char operation[5] = "read";

    fat_fuse_log_activity(operation, file);
...
/* remove empty directory for @path */
 int fat_fuse_rmdir (const char *path){
    errno = 0;
    fat_file dir;
    fat_file parent_dir;
    fat_volume vol = get_fat_volume(); //obtenemos el volumen.
    fat_tree_node parent_node = fat_tree_node_search(vol->file_tree, path); //buscamos el nodo que contiene al directorio segun su path
    if(parent_node == NULL){ //verificamos si el path es valido o el arbol existe, retornamos error
        errno = ENOENT;
        return -errno;
    }
    dir = fat_tree_get_file(parent_node); //obtenemos el archivo del nodo
    parent_dir = fat_tree_get_parent(parent_node);
    if(dir == NULL){ //si no existe o no se pudo obtener.
        errno = ENOENT; 
        return -errno;
    }
    if(!fat_file_is_directory(dir)){ //verificamos si el fat_file obtenido es un directorio
        fat_error("Error! Parent is not directory\n");
        errno = ENOTDIR;
        return -errno;
    }
    if(dir->dir.nentries > 2 ){ //error si el directorio no esta vacio :(
       fat_error(" not empty ");
       errno = ENOTEMPTY;
       return -errno;
    }

    fat_file_remove(dir, parent_dir); 
    
    vol->file_tree = fat_tree_delete(vol->file_tree, path); // se elimina el nodo que contiene fat_file del tree :().
    return 0;
}
...
int fat_fuse_unlink(const char *path){
    ...
    fat_file parent_dir;
    fat_volume vol = get_fat_volume(); //obtenemos el volumen.
    fat_tree_node file_node = fat_tree_node_search(vol->file_tree, path); //buscamos el nodo que contiene al directorio segun su path
    if(file_node == NULL){ //verificamos si el path es valido o el arbol existe, retornamos error
        errno = ENOENT;
        return -errno;
    }
    file = fat_tree_get_file(file_node); //obtenemos el archivo del nodo
    parent_dir = fat_tree_get_parent(file_node); //obtenemos el directorio que contiene a file
    if(file == NULL){ //si no existe o no se pudo obtener.
        errno = ENOENT; 
        return -errno;
    }
    if(fat_file_is_directory(file)){ //verificamos si el fat_file obtenido es un archivo
        fat_error("Error! file is directory\n");
        errno = EISDIR;
        return -errno;
    }
    fat_file_remove(file, parent_dir); //eliminamos el archivo,(liberamos clusters que ocupaba file) y actualizamos parent_dir
    vol->file_tree = fat_tree_delete(vol->file_tree, path); // se elimina el nodo que contiene fat_file del tree :().
    return 0;
}
