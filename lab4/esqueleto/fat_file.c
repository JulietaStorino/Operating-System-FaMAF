...
static void read_cluster_dir_entries(u8 *buffer, fat_dir_entry end_ptr,
    ...
    for (disk_dentry_ptr = (fat_dir_entry)buffer; disk_dentry_ptr <= end_ptr;
        ...
        if(disk_dentry_ptr->extension[0] == 'l' )
...
void fat_file_remove(fat_file file, fat_file parent) {
    ...
    last_cluster = file->start_cluster; //asignamos como actual cluster al primer cluster donde empieza el file

    if (errno != 0) { // si se produce algun error
        return;
    }
    // Marcar los clusters siguientes como no utilizados
    while (!fat_table_is_EOC(file->table, last_cluster)) {//mientras el cluster actual no sea el final de la cadena
        // Si hubo un error, continuamos con la función.
        next_cluster = fat_table_get_next_cluster(file->table, last_cluster);//se obtiene el numero del siguiente cluster en la secuencia de clusters del archivo
        fat_table_set_next_cluster(file->table, last_cluster, FAT_CLUSTER_FREE);//marcar al cluster actualc como libre
        last_cluster = next_cluster; //se actualiza el last_cluster
    }
    write_dir_entry(parent, file->dentry, file->pos_in_parent);
}

static u32 fat_get_next_or_add_cluster(fat_file file, u32 cluster){

    u32 next_clus = fat_table_get_next_cluster(file->table, cluster);

    if( fat_table_is_EOC(file->table, cluster)){
        DEBUG("Adding a new cluster");

        next_clus = fat_table_get_next_free_cluster(file -> table);
        fat_table_set_next_cluster(file->table, cluster, next_clus);
        fat_table_set_next_cluster(file->table, next_clus, FAT_CLUSTER_END_OF_CHAIN);
        //Error handling
        if (errno != 0) {
        return 0;
        }
...
ssize_t fat_file_pwrite(fat_file file, const void *buf, size_t size,
    ...
    while (bytes_remaining > 0 && !fat_table_is_EOC(file->table, cluster)) {
        ...
        if (bytes_remaining > 0) {
            cluster = fat_get_next_or_add_cluster(file, cluster);
            if (errno != 0 || !fat_table_is_valid_cluster_number(file->table, cluster)) {
                break;
            }
        }
    }
